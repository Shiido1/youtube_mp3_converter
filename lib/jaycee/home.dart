...
class _MyHomePageState extends State<MyHomePage> {
  // create a temporer list of items
  final _images = [
    {
  ...
  /// get last name in a link
  String _getStringPathName(String name) {
                  ...
                  : DownloadItem(
                      ...
                      onItemClick: (task) {
                        if (task.status == DownloadTaskStatus.complete) {
                          print(
                              'ItemPath: $_localPath/${_getStringPathName(task.link)}');
                          // *  You can now open the music or media with your desired options
                          // *  Since it is now cached in the device memory
                        }
                      },
                      onAtionClick: (task) => _performTasl(task),
                    ))
              .toList(),
        ),
      );
...
//* cancels an ongoing download using the task id
  void _cancelDownload(_TaskInfo task) async {
    await FlutterDownloader.cancel(taskId: task.taskId);
  }

//* pauses an ongoing download using the task id
  void _pauseDownload(_TaskInfo task) async {
    await FlutterDownloader.pause(taskId: task.taskId);
  }

//* resumes an ongoing download using the task id
  void _resumeDownload(_TaskInfo task) async {
    String newTaskId = await FlutterDownloader.resume(taskId: task.taskId);
    task.taskId = newTaskId;
  }

//* retries an ongoing download using the task id
  void _retryDownload(_TaskInfo task) async {
    String newTaskId = await FlutterDownloader.retry(taskId: task.taskId);
    task.taskId = newTaskId;
  }

//* opens an already or completed download
  Future<bool> _openDownloadedFile(_TaskInfo task) {
    return FlutterDownloader.open(taskId: task.taskId);
  }

//* deletes and already downloaded or completed download
...
//* prepares the items we wish to download
  Future<Null> _prepare() async {
    ...
    int count = 0;
    _tasks = []; // creates an empty task list
    _items = []; // creates and empty items list

//* adds our already crated image list to the empty task list we created as this will be the tasks we are working on
...
// assigns each tasks we are to work with, each with a respective id, url, progress
    tasks?.forEach((task) {
...
//* finds available space for storage on users device
  ...
  void _performTasl(_TaskInfo task) {
...
//  A private model class to hold task info
class _TaskInfo {
  final String name;
  final String link;
...
//  A private model class to hold items
class _ItemHolder {
  final String name;
